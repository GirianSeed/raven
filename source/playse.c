...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf("usage: raven [-d] [-r] [-s se_id] -o outpath -e encoder efx wvx [wvx2] [wvx3]\n");
        return 1;
    }
    ...
    if (!efx)
    {
        printf("error: missing required argument efx\n");
        return 1;
    }
    ...
    for (int i = min; i < max; i++)
    {
        ...
        sd_set_cli((0 << 16) | (63 << 8) | i, SD_ASYNC);
